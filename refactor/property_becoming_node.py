...
class PropertyBecomingNode:
     def __init__(self, importer, kandidat_properti, pbn_configs, limit=100):
     ...
     def execute(self):
        for prop in self.kandidat_properti:
            # 2. Ekstrak nama relasi dan prefix dari pbn_configs
            ...
            for row in rows:
                ...
                # 3. Panggil generate_labels buatanmu menggunakan parameter value dan prefix
                ...
                for label in labels:
                    ...
                    // 4. FIX: Ubah 'target' menjadi 'node' agar merujuk pada hasil YIELD di atas
                ...
                # Query hapus properti setelah node dan relasi terbentuk
                remove_query = f"""
                MATCH (n)
                WHERE id(n)=$node_id
                REMOVE n.{prop}
